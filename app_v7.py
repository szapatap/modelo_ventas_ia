"""
APLICACIÓN STREAMLIT - Sales Intelligence Platform v5
Frontend profesional con Sidebar y tema oscuro
VERSIÓN CORREGIDA v5 - Fix Guardado de Predicciones + Menú Reordenado
"""
...
# ==========================================
# GESTIÓN DE BD Y USUARIOS - v5 MEJORADO
# ==========================================

def init_db():
    """Inicializa la BD con estructura correcta - v5"""
    try:
        ...
        c.execute("DROP TABLE IF EXISTS predictions_old")
        
        # Crear tabla predictions con estructura correcta
...
def save_prediction_db(user, client, zone, category_product, prob, label, recommendation):
    """Guarda predicción - v5 SIMPLIFICADA Y ROBUSTA"""
    try:
        conn = sqlite3.connect('sales_app.db')
        c = conn.cursor()
        
        # Insertar con estructura correcta (siempre la misma)
        c.execute('''INSERT INTO predictions
        (user_id, client_name, zone, category_product, prob, label, recommendation) 
        VALUES (?,?,?,?,?,?,?)''',
        (user, client, zone, category_product, float(prob), label, recommendation))
        
        conn.commit()
        conn.close()
        
        print(f"✅ Predicción guardada: {client} - {category_product}")
        return True
...
def main():
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h1 style="color: #FFF; margin: 0;">📊 Sistema Predictivo de Ventas v5</h1>
                ...
                if st.button("🚀 Predecir Adjudicación", use_container_width=True, type="primary"):
                    if client:
                        ...
                        if saved:
                            ...
                            st.error("❌ Error al guardar predicción. Verifica la BD ejecutando:\n\npython debug_bd.py")
        else:
