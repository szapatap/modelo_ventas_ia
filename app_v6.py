...
VERSIÓN CORREGIDA v4 - Fix BD Migration + Guardado de Predicciones
"""
...
# ==========================================
# GESTIÓN DE BD Y USUARIOS - v4 MEJORADO
# ==========================================

def init_db():
    """Inicializa o migra la BD - VERSIÓN MEJORADA CON MANEJO DE ERRORES"""
    ...
    try:
        ...
        # Verificar si tabla predictions existe
        ...
        if not table_exists:
            ...
            zone TEXT, category_product TEXT, prob REAL, label TEXT, recommendation TEXT, 
            ...
            needs_add_columns = False
            
            # Caso 1: BD antigua con 'categoria'
            if 'categoria' in columns and 'category_product' not in columns:
                needs_migration = True
            
            # Caso 2: BD parcialmente actualizada
            if 'category_product' in columns and 'recomendacion' in columns and 'recommendation' not in columns:
                needs_add_columns = True
            
            if needs_migration:
                # MIGRACIÓN COMPLETA DE TABLA ANTIGUA
                try:
                    ...
                    # Copiar datos desde tabla antigua
                    c.execute('''INSERT INTO predictions 
                    (id, user_id, client_name, zone, category_product, prob, label, recommendation, timestamp)
                    SELECT id, user_id, client_name, zone, 
                           COALESCE(categoria, 'Producto General'), 
                           prob, label, 
                           COALESCE(recomendacion, 'Sin recomendación'), 
                           timestamp
            ...
            elif needs_add_columns:
                # AGREGAR COLUMNAS FALTANTES
                try:
                    if 'category_product' not in columns:
                        c.execute("ALTER TABLE predictions ADD COLUMN category_product TEXT DEFAULT 'Producto General'")
                    
                    if 'recommendation' not in columns:
                        c.execute("ALTER TABLE predictions ADD COLUMN recommendation TEXT DEFAULT 'Sin recomendación'")
                ...
                # BD ya está actualizada, no hacer nada
                pass
...
def save_prediction_db(user, client, zone, category_product, prob, label, recommendation):
    ...
    try:
        ...
        if 'category_product' in columns and 'recommendation' in columns:
            # Estructura nueva
            c.execute('''INSERT INTO predictions
            (user_id, client_name, zone, category_product, prob, label, recommendation) 
            VALUES (?,?,?,?,?,?,?)''',
            (user, client, zone, category_product, prob, label, recommendation))
        
        elif 'categoria' in columns and 'recomendacion' in columns:
            # Estructura antigua
            c.execute('''INSERT INTO predictions
...
def get_history(user_id=None, days=30):
    """Obtiene histórico - COMPATIBLE CON AMBAS ESTRUCTURAS"""
    try:
        conn = sqlite3.connect('sales_app.db')
        ...
        if 'categoria' in df.columns and 'category_product' not in df.columns:
            df.rename(columns={'categoria': 'category_product'}, inplace=True)
        
        if 'recomendacion' in df.columns and 'recommendation' not in df.columns:
            df.rename(columns={'recomendacion': 'recommendation'}, inplace=True)
...
def main():
    ...
    with st.sidebar:
        ...
        menu = st.radio(
            "Menú Principal",
            ["⚙️ Entrenamiento", "📊 Dashboard", "📈 Análisis", "👤 Vendedores", 
             "👥 Clientes", "🔮 Predicción", "📝 Historial"],
            key='menu_principal'
        )
    ...
    if menu == "⚙️ Entrenamiento":
        ...
        if st.session_state['df'] is not None:
            ...
            if not df_hist.empty:
                ...
                available_cols = []
                for col in ['client_name', 'zone', 'category_product', 'prob', 'label', 'timestamp']:
                    if col in df_hist.columns:
                        available_cols.append(col)
                
                if available_cols:
                    st.dataframe(df_hist[available_cols], use_container_width=True, height=400)
                else:
            ...
            if model is None:
                ...
                if st.button("🚀 Predecir Adjudicación", use_container_width=True, type="primary"):
                    if client:
                        ...
                        # Guardar y verificar
                        saved = save_prediction_db(st.session_state['user'], client, zona, categoria, prob, label, recomendacion)
                        
                        if saved:
                            st.success("✅ Predicción guardada en el historial")
                        else:
                            st.warning("⚠️ Predicción realizada pero no se pudo guardar en historial. Verifica la BD.")
        else:
