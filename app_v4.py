...
VERSIÓN CORREGIDA v2 - Fix SQLite + Reorden Menú
"""
...
# ==========================================
# GESTIÓN DE BD Y USUARIOS - CORREGIDO
# ==========================================

def init_db():
    """Inicializa la BD con las columnas correctas"""
    conn = sqlite3.connect('sales_app.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, password TEXT)''')
    
    # Crear tabla con columnas correctas (categoria en minúscula)
...
def save_prediction_db(user, client, zone, category_product, prob, label, recommendation):
    """Guarda predicción con nombres de columnas correctos"""
    conn = sqlite3.connect('sales_app.db')
    c = conn.cursor()
...
def get_history(user_id=None, days=30):
    """Obtiene histórico con nombres correctos de columnas"""
    conn = sqlite3.connect('sales_app.db')
...
def main():
    ...
    # MENÚ DE NAVEGACIÓN - NUEVO ORDEN
    ...
    if menu == "⚙️ Entrenamiento":
        ...
        if st.session_state['df'] is not None:
            ...
            if not df_hist.empty:
                ...
                cols_show = ['client_name', 'zone', 'category_product', 'prob', 'label', 'timestamp']
                st.dataframe(df_hist[cols_show], use_container_width=True, height=400)
            else:
