...
VERSIÓN CORREGIDA v3 - Compatible con BD antigua y nueva
"""
...
# ==========================================
# GESTIÓN DE BD Y USUARIOS - v3 COMPATIBLE
# ==========================================

def init_db():
    """Inicializa o migra la BD - COMPATIBLE CON VERSIONES ANTERIORES"""
    ...
    if not table_exists:
        ...
        # Tabla existe, verificar si tiene columna category_product
        c.execute("PRAGMA table_info(predictions)")
        columns = [column[1] for column in c.fetchall()]
        
        # Si no tiene category_product pero tiene categoria, migrar
        if 'category_product' not in columns and 'categoria' in columns:
            ...
            # Copiar datos (categoria → category_product)
            c.execute('''INSERT INTO predictions 
            (id, user_id, client_name, zone, category_product, prob, label, recommendation, timestamp)
            SELECT id, user_id, client_name, zone, categoria, prob, label, recomendacion, timestamp
            FROM predictions_old''')
            
            # Eliminar tabla antigua
            c.execute("DROP TABLE predictions_old")
        
        # Si no tiene recommendation pero tiene recomendacion, migrar
        if 'recommendation' not in columns and 'recomendacion' in columns:
            c.execute('''ALTER TABLE predictions 
            ADD COLUMN recommendation TEXT DEFAULT NULL''')
            
            c.execute('''UPDATE predictions 
            SET recommendation = recomendacion 
...
def save_prediction_db(user, client, zone, category_product, prob, label, recommendation):
    """Guarda predicción - COMPATIBLE"""
    try:
        conn = sqlite3.connect('sales_app.db')
        c = conn.cursor()
        
        # Intentar con columnas nuevas
        try:
            c.execute('''INSERT INTO predictions
            ...
            # Si falla, intentar con columnas antiguas
            c.execute('''INSERT INTO predictions
            ...
            if model is None:
                ...
                if st.button("🚀 Predecir Adjudicación", use_container_width=True, type="primary"):
                    if client:
                        ...
                        # Guardar con manejo de compatibilidad
                        if not save_prediction_db(st.session_state['user'], client, zona, categoria, prob, label, recomendacion):
                            st.warning("⚠️ Predicción realizada pero no se pudo guardar en historial")
                        else:
